...
task :find_tickets do
  ...
  messages_to_track = JSON.parse(redis.get('messages_to_track')) || {}
  ...
  messages_to_track.each do |identifier, message|
    ...
    message[:origin] = message["origin"] || message[:origin]
    message[:destination] = message["destination"] || message[:destination]
